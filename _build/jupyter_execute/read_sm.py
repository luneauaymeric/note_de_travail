...
dict_sentence = []
index_name = []
form_to_keep = pd.read_csv(f"{path_base}forme_exclue_a_sauver.csv", sep = ",")
...
for n, t in enumerate(df0["text"][0:100000]):
    ...
    if len(dict_sentence) < 16 :
        ...
        marker2 = f"{x}"
        for y in form_to_keep[x].dropna().unique():
            #print(y)
            marker.append(f"(?:^|\W){y}(?:$|\W)")
...
count_marker_style = count_marker.style.format(precision=0, na_rep='').set_caption("Le nombre de marqueurs trouvés selon la méthode utilisée") .set_table_styles([{
     'selector': 'caption',
...
dfbiom1_style = dfbiom1.style.format(precision=0, na_rep='').set_table_styles([cell_hover, index_names, headers]).set_properties(**{'text-align': 'left'}).set_caption("Les formes comptées comme occurrences par la méthode 1 et exclues avec la méthodes 2") .set_table_styles([{
     'selector': 'caption',
     'props': 'caption-side: top ; color: black ; font-size : 14pt'
 }], overwrite=False)
